...
WaitForHotGoal::WaitForHotGoal() {
	...
	waitval = std::max(6 - Robot::prefs->GetFloat("autonomousDriveTimeout", AutonomousDriveCommand::timeoutDflt),0.1f);
	printf("Hot Goal Wait Val: %f\n", waitval);
	SetTimeout(waitval);
}
...
bool WaitForHotGoal::IsFinished() {
	...
	printf( "isHot = %s, timedOut = %s\n", isHot ? "true" : "false", IsTimedOut() ? "true" : "false" );
