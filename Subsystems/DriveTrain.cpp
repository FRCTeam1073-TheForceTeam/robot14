...
bool usingCurrent = Robot::prefs->GetBoolean("currentMode",true);
float currentConstant = Robot::prefs->GetFloat("ampsConstant",40.0f);
bool changedToVoltage = false;
bool changedToCurrent = false;
...
void DriveTrain::MecanumDriveAction(float joystickX, float joystickY, float joystickTwist) {
	...
	if(!changedToVoltage&&!usingCurrent)
	{
		rightBack->ChangeControlMode(SmartCANJaguar::kPercentVbus);
		leftBack->ChangeControlMode(SmartCANJaguar::kPercentVbus);
		rightFront->ChangeControlMode(SmartCANJaguar::kPercentVbus);
		leftFront->ChangeControlMode(SmartCANJaguar::kPercentVbus);
		rightBack->EnableControl();
		leftBack->EnableControl();
		rightFront->EnableControl();
		leftFront->EnableControl();
		changedToVoltage = true;
		changedToCurrent = false;
	}
	if(!changedToCurrent&&usingCurrent)
	{
		rightBack->ChangeControlMode(SmartCANJaguar::kCurrent);
		leftBack->ChangeControlMode(SmartCANJaguar::kCurrent);
		rightFront->ChangeControlMode(SmartCANJaguar::kCurrent);
		...
		rightFront->EnableControl();
		leftFront->EnableControl();
		changedToVoltage = false;
		changedToCurrent = true;
	}
	//start of ramping
